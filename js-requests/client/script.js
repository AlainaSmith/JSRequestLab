...
/*
    ...
    Handle the promise that's returned with a .then, which you should 
    pass a callback function to. Inside the callback function, console.log the 
    response's data (in the intermediate instructions we'll come back to this 
...
const ohMy = () => {
    ...
    .then((response) =>{
       ...
       const bodyPlacement = document.querySelector('body')
       bodyPlacement.append(animal)
    } 
    })
}
//const textRepeat = document.querySelector('#repeat-text')
...
/*
    ...
    The function that runs when this request is made will return whatever parameter you 
    sent 
...
const repeatMyParam = event => {
    axios.get(`http://localhost:3000/repeat/sunshine`)
    .then((response) => {
        const textRepeat = document.querySelector('#repeat-text')
        textRepeat.textContent = response.data
...
const getRequest = event => {
    axios.get('http://localhost:3000/query-test/?name=alaina')
    .then((response) => {
        console.log(response.data)
    })
}
...
// PROBLEM 10 
/*
    In the function that you wrote for Problem 8, change the URL to test a couple 
    different scenarios. 
...
/*
    ...
    input field and a button. The input field should have a placeholder that tells 
    the user to enter a food. And the button should indicate that it will add food 
    into a list. 

    In this file (script.js), create a function called createFood. 
    
    Inside the function, select the input you just created in the HTML and save it 
    to a variable called foodInput. 
...
foodInput.addEventListener('submit', createFood)

// Next, create an object called body inside the function. It should have one 
// key-value pair. The key should be newFood (make sure to match the case and 
...
// Pass a callback function to the .then. Inside that callback, console log 
// the res.data. 

// Based on what we did earlier to display this type of data, write code that 
// will display the response in your HTML document. 


// for (let i = 0; i < response.data.length; i++){
//     foodInput(reponse.data[i])
// }